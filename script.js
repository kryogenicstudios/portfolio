...
const ratingAverageLarge = document.querySelector("[data-rating-average-large]");
const ratingCount = document.querySelector("[data-rating-count]");
const ratingCountLabel = document.querySelector("[data-rating-count-label]");
const ratingSummaryCopy = document.querySelector(".rating-summary-copy");

const RATING_STORAGE_KEY = "kryosphere-ratings";
const DEFAULT_AVATAR =
  "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 80 80'%3E%3Crect width='80' height='80' rx='18' fill='%23161a24'/%3E%3Ccircle cx='40' cy='31' r='14' fill='%237c7cff'/%3E%3Cpath d='M20 67c2-12 11-18 20-18s18 6 20 18' fill='%234da3ff'/%3E%3C/svg%3E";
...
const readSavedRatings = () => {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(RATING_STORAGE_KEY) || "[]");
...
const renderRatingSummary = (ratings) => {
  const totalRatings = ratings.length;
  const averageValue = totalRatings > 0
    ? (ratings.reduce((sum, item) => sum + Number(item.score || 0), 0) / totalRatings).toFixed(1)
    : "5.0";
  const ratingLabel = totalRatings === 1 ? "1 Rating" : `${totalRatings} Ratings`;

  if (ratingAverage) {
    ratingAverage.textContent = averageValue;
  }

  if (ratingAverageLarge) {
    ratingAverageLarge.textContent = averageValue;
  }

  if (ratingCount) {
    ratingCount.textContent = String(totalRatings);
  }

  if (ratingCountLabel) {
    ratingCountLabel.textContent = ratingLabel;
  }

  if (ratingSummaryCopy) {
    ratingSummaryCopy.textContent = totalRatings > 0
      ? `Based on ${totalRatings} saved ${totalRatings === 1 ? "rating" : "ratings"} in this browser.`
      : "No ratings yet.";
  }
};
...
const renderRatings = () => {
  ...
  const ratings = readSavedRatings()
    .slice()
  ...
  if (ratings.length === 0) {
    ratingList.innerHTML = `
      <div class="rating-empty">
        <p class="rating-empty-title">No ratings saved yet.</p>
        <p class="rating-empty-copy">Enter a Roblox username and submit the first one.</p>
      </div>
    `;
    return;
  }

  ratingList.innerHTML = ratings.map((entry) => `
    <article class="rating-entry">
      <img class="rating-entry-avatar" src="${escapeHtml(entry.avatarUrl || DEFAULT_AVATAR)}" alt="${escapeHtml(entry.displayName)} avatar">
      <div class="rating-entry-body">
        <div class="rating-entry-top">
          <div>
            <p class="rating-entry-name">${escapeHtml(entry.displayName)}</p>
            <p class="rating-entry-meta">@${escapeHtml(entry.username)} &bull; ${escapeHtml(new Date(entry.createdAt).toLocaleDateString("en-GB"))}</p>
          </div>
          <p class="rating-entry-score">${"&#9733;".repeat(Number(entry.score))}<span>${"&#9733;".repeat(5 - Number(entry.score))}</span></p>
        </div>
        <p class="rating-entry-message">${escapeHtml(entry.message)}</p>
      </div>
    </article>
  `).join("");
};

const setRatingStatus = (message, tone = "neutral") => {
  if (!ratingStatus) {
    return;
  }
...
if (ratingForm) {
  const usernameInput = ratingForm.elements.namedItem("username");

  usernameInput?.addEventListener("input", () => {
    setRatingStatus("Enter your Roblox username and save the rating.", "neutral");
  });
  ...
  ratingForm.addEventListener("submit", (event) => {
    ...
    if (!username) {
      setRatingStatus("Enter a Roblox username before saving.", "error");
      return;
    }

    if (!message || score < 1 || score > 5) {
      setRatingStatus("Complete the rating fields before saving.", "error");
      return;
    }
    ...
    if (existingIndex >= 0) {
      ratings[existingIndex] = nextEntry;
    } else {
      ratings.push(nextEntry);
    }

    saveRatings(ratings);
    renderRatings();
    ratingForm.reset();
    setRatingStatus("Rating saved on this device.", "success");
  });
}
